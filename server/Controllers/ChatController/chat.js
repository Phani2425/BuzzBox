...
exports.getMyChats = async (req, resp) => {
  try {
    const chats = await Chat.find({ members: { $in: [req.user.id] } }).populate(
      "members",
      "userName profilePic"
    );
    ...
    const transfomedChat = chats.map(({ _id, name, members, groupChat }) => {
      ...
      return {
        ...
        grpAvatar: groupChat
          ? members.slice(0, 3).map((member) => member.profilePic)
          : [otherMemeber.profilePic],
      };
    });
...
exports.addMembers = async (req, resp) => {
  try {
    ...
    emitEvent(
      ...
      ALERT,
      uniqueMembers,
      `You are added to ${chat.name} group..🤗`
    );
  ...
  try {
    ...
    if (!group || !group.groupChat) {
      return resp.status(404).json({
        success: false,
        message:
          "chat with passed id is not found or the chat is not a group chat",
      });
    }
...
exports.leaveGroup = async (req, resp) => {
  try {
    ...
    emitEvent(
      ...
      ALERT,
      remainingMembers,
      `user ${user.userName} has left the ${chat.name} group`
    );
...
exports.sendAttachments = async (req, resp) => {
  try {
    ...
    const formatedAttachments = attachments.map(
...
exports.deleteChat = async (req, resp) => {
  try {
    ...
    const attachmentDeletionPromises = messages.flatMap((message) =>
      message.attachments.map(async (attachment) => {
        try {
          ...
          console.log(
            `Attachment ${attachment.public_id} deletion result:`,
            result
          );
          return result;
        } catch (error) {
          console.error(
            `Error deleting attachment ${attachment.public_id}:`,
            error
          );
          return null;
        }
    ...
    const deletionResults = await Promise.allSettled(
      attachmentDeletionPromises
    );

    // Optional: Log detailed results
    const deletionSummary = {
      total: deletionResults.length,
      deleted: deletionResults.filter(
        (r) => r.status === "fulfilled" && r.value && r.value.result === "ok"
      ).length,
      notFound: deletionResults.filter(
        (r) =>
...
exports.getMessagesAfterMyLastSeen = async (req, resp) => {
  try {
    const userId = req.user.id;
    ...
    const userLastSeen = user.lastSeen;
    //find messages save in db after the last seen of the user
    const messages = await Message.find({ createdAt: { $gt: userLastSeen } });

    // Group messages by chatId and count the number of messages for each chatId
    const chatIdMap = new Map();

    messages.forEach((message) => {
      const chatId = message.chat.toString();
      if (chatIdMap.has(chatId)) {
        chatIdMap.set(chatId, chatIdMap.get(chatId) + 1);
      } else {
        chatIdMap.set(chatId, 1);
      }
    });

    // Convert the Map to an array of objects
    const result = Array.from(chatIdMap, ([chatId, count]) => ({
      chatId,
      count,
    }));
    ...
    console.log(
      "error occured while fetching the unread messages",
      err.message
    );
    console.error(err.message);
